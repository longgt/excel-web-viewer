...
export function parseExcelWorkbook(data: ArrayBuffer, fileName: string, fileSize: number): WorkbookData {
  ...
  for (const sheetName of sheetNames) {
    ...
    // Convert to 2D array: array of rows
    const rawJsonRows = XLSX.utils.sheet_to_json(worksheet, {
      header: 1,
      raw: false, // get formatted text values where applicable
      defval: '',
    }) as CellValue[][];
    ...
    // Check if the first row looks like headers (all non-empty strings or populated values)
    let headers: string[] = [];
